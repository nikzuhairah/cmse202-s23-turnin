...
class Animal_Agent():
    ...
    def __init__(self,xmax=100,ymax=100,ptype="Prey",sex="M",age_limit=50,catch_radius=10,mate_range=5,gestation=8,last_litter_time=-8, shape="o",color="saddlebrown",saturation_pop=150, gender=None):
        '''
    ...
    def procreate(self,all_agents,time):
        ...
        if (time - self.last_litter_time >= self.gestation):
            for agent in all_agents:
                if (agent.ptype == self.ptype) and (agent.age < agent.age_limit) and (not agent.is_caught):
                    if self.get_distance(agent) < self.mate_range:
                        self.last_litter_time = time
                        new_agent = Animal_Agent(ptype=self.ptype,
                                                 sex=random.choice(["M","F"]),
                                                 age_limit=agent.age_limit,
                                                 catch_radius=agent.catch_radius,
                                                 mate_range=agent.mate_range,
                                                 gestation=agent.gestation,
                                                 color=self.color,
                                                 shape=self.shape,
                                                 saturation_pop=self.saturation_pop)
                        new_agent.x = self.x
                        new_agent.y = self.y
                        return new_agent
        return False
